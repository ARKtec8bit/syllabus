...
function parseKarateData(xmlDoc) {
  ...
  for (const category of categories) {
    ...
    for (const item of items) {
      ...
      const videoLink =
        item.getElementsByTagName("video_link")[0]?.textContent || ""; // Check if video_link exists
...
function initializeApp(karateData) {
  ...
  document
    ...
    .addEventListener("change", () => updateNameCombo(karateData));
  document
    .getElementById("level")
    .addEventListener("change", () => updateNameCombo(karateData));
  document
    .getElementById("name")
    .addEventListener("change", () => showDetails(karateData));
...
function updateNameCombo(karateData) {
  ...
  if (category) {
    const items = karateData[category].filter(
